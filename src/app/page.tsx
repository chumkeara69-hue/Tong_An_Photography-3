...
export default async function Home() {
  ...
  const photosWithUrls = await Promise.all(
    photos.map(async (photo) => {
      let imageUrl = photo.previewStorageKey;

      if (
        photo.previewStorageKey &&
        !photo.previewStorageKey.startsWith("/")
      ) {
        try {
          imageUrl = await createDownloadUrl(photo.previewStorageKey);
        } catch {
          imageUrl = "/version-3-preview.png";
        }
      }
  ...
  return (
    <main>
      <section className="hero">
        <div className="container hero-inner">
          <div className="hero-copy">
            ...
            <p className="lead">
              Original photography of Cambodia — landscapes, people,
              architecture, culture, and the quiet details of everyday life.
              Browse the collection and license high-quality digital images.
            </p>
          ...
          {photosWithUrls[0] ? (
            <Link
              href={`/photos/${photosWithUrls[0].slug}`}
              className="hero-image-wrap"
              aria-label={`View ${photosWithUrls[0].title}`}
            >
              <img
                className="hero-image"
                src={photosWithUrls[0].imageUrl}
                alt={photosWithUrls[0].title}
              />

              <span className="hero-caption">
                <strong>{photosWithUrls[0].title}</strong>
                <small>
                  {photosWithUrls[0].category.name} · View photograph →
                </small>
              </span>
            </Link>
          ) : (
            <div className="hero-image-wrap">
              <img
                className="hero-image"
                src="/version-3-preview.png"
                alt="Tong An Photography — Cambodia"
              />
            </div>
          )}
        </div>
      ...
      <section
        ...
        aria-label="Why buy from Tong An Photography"
      >
        <div>
          <strong>Original work</strong>
          <span>Captured by Tong An</span>
        </div>

        <div>
          <strong>High-quality files</strong>
          <span>Digital download after approval</span>
        </div>

        <div>
          <strong>Simple licensing</strong>
          <span>Ask about commercial use</span>
        </div>

        <div>
          <strong>Human verified</strong>
          <span>Payment reviewed before download</span>
        </div>
      </section>
      ...
      <section className="container section section-tight">
        <div className="section-intro">
          <div>
            ...
            <p className="muted section-description-left">
              A curated starting point. Open any image for details, licensing
          ...
          <div className="section-intro centered">
            <div>
              ...
              <p className="muted section-description">
                Start with a subject, then discover the places, people and
                details that make Cambodia unforgettable.
              </p>
            </div>
          </div>
      ...
      <section className="container section about-strip">
        <div className="about-copy">
          ...
          <p className="lead">
            Every photograph is a small record of a real moment. Tong An
            Photography focuses on authentic Cambodian scenes and timeless
            images made to be remembered.
          </p>
